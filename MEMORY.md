...
## 老大 P0 规则（2026-06-30 21:00 拍板，所有 bot 适用）

1. **每个 bot 改完代码后，必须【主动】提交到本地仓库**，不要等老大说"提交"才开始动；不需要每次都 push 到远程仓库，**本地 commit 就已留痕不会丢文件**。
2. 提交内容（commit message 格式）必须说清三件事：
...
3. **如有 push，推送时格式与本地 commit 保持一致**（commit=push 同格式）；平时只需要本地 commit 即可，不需要每次都 push。
...
5. **规则更新历史**：v1 (16:30) 为"commit + 每次 push"；v2 (21:00) 老大明确"不需要每次 push，本地 commit 就留痕了"，同步把 P0 第 1/3 条改为"只 commit，push 时按 commit 格式"。
